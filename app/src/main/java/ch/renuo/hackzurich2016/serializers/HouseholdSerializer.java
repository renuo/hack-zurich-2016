...
public class HouseholdSerializer {
    ...
    private List<Object> getSystemAlarms() {
        ...
        for (Cluster cluster : household.getClusters()) {
            for (Device device : cluster.getDevices()) {
                for (SystemAlarm systemAlarm : device.getSystemAlarms()) {
                    systemAlarms.add(getSystemAlarm(cluster.getId(), device.getId(), systemAlarm));
                }
            }
        }
        return systemAlarms;
    ...
    @NonNull
    private Map<String, Object> getSystemAlarm(UUID clusterId, UUID deviceId, SystemAlarm systemAlarm) {
        Map<String, Object> map = new HashMap<>();
        map.put("id", systemAlarm.getId());
